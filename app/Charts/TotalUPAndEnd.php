...
class TotalUPAndEnd
{
    ...
    public function __construct(LarapexChart $TotalUPAndEnd)
    {
        $this->chart = $TotalUPAndEnd;
    }
    ...
    public function build(): LineChart 
    {
        ...
        $data = $data->where(function ($query) use ($currentYear, $currentMonth, $currentDate) {
            $query->whereYear('start_datetime', '=', $currentYear)
                ->where(function ($query) use ($currentMonth, $currentDate) {
                    $query->whereMonth('start_datetime', '>=', $currentMonth)
                        ->orWhere(function ($query) use ($currentMonth, $currentDate) {
                            $query->whereMonth('start_datetime', '=', $currentMonth)
                                ->whereDay('start_datetime', '>', $currentDate);
                        });
                });
        })->select(
