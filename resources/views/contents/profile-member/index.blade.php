...
@section('title', 'Update Profile Member')
    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"> Profile Member</li>
            </ol>
        </nav>
    </div>
</div>
...
<div class="row">
    <div class="col-lg-12 col-md-12 col-md-12">
        <div class="card blog-edit">
            <form id="form" action="{{ route('profile-member.update', $member->id) }}" method="POST" enctype="multipart/form-data">
                <div class="card-body">
                    @csrf
                    @method('PUT')
                    <input id="id" name="id" type="hidden" value="">
                    <h5 class="bg-gray mt-4">INFORMASI {{ strtoupper(Auth::user()->member->member_type->name ?? '') }}</h5>
                    <div class="form-group mb-4">
                        <label class="form-label text-dark">Nama {{ ucwords(Auth::user()->member->member_type->name ?? '') }}</label>
                    ...
                    <div class="form-group mb-4">
                        <label class="form-label text-dark">Email {{ ucwords(Auth::user()->member->member_type->name ?? '') }}</label>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <div class="row pt-4 border-1 border">
                            ...
                            <div class="col-lg-3 mb-3">
                                <label class="ckbox">
                                    <input type="checkbox" name="category_id[]" value="{{ $item->id }}" @if(in_array($item->idz, $selectedCategoryIds)) checked @endif> <span>{{ $item->name }}</span>
                                </label>
                            </div>
                            @endforeach
                        </div>
                    ...
                    <h5 class="bg-gray mt-4">INFORMASI PIMPINAN {{ strtoupper(Auth::user()->member->member_type->name ?? '') }}</h5>
                    <div class="mb-3">
                    ...
                    <h5 class="bg-gray mt-4 text-uppercase">INFORMASI KELENGKAPAN DOKUMEN {{ Auth::user()->member->member_type->name ?? '' }}</h5>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label class="form-label text-dark">Name {{ ucwords(Auth::user()->member->member_type->name ?? '') }} Di NIB / Akte Perusahaan</label>
