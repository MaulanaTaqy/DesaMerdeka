    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"> Blog Banner</li>
            </ol>
        </nav>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        table = $("#datatable").DataTable({
            ...
            serverSide: true,
            autoWidth: false,
            ajax: "{{ route('blog-banner.datatable') }}",
            columnDefs: [
            {
                targets: 0,
    ...
    function edit(id){
        ...
        var url = "{{ route('blog-banner.edit',":id") }}";
    ...
    var url = "{{ route('blog-banner.store') }}";
    
    $('#btnSave').text('Menyimpan...');
    $('#btnSave').attr('disabled', true);

    if(submit_method == 'edit'){
        url = "{{ route('blog-banner.update',':id') }}";
        url = url.replace(':id', id);
    ...
    function destroy(id) {
        var url = "{{ route('blog-banner.destroy',":id") }}";
        url = url.replace(':id', id);
