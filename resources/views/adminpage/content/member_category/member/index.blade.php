    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"> Member Category</li>
            </ol>
        </nav>
    </div>
</div>
        ...
        <div class="card card-xl-stretch mb-xl-8">
            ...
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bold fs-3 mb-1">List Member Category </span>
                </h3>
            ...
            <div class="modal-content modal-content-demo">
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        ...
                        <button  id="btnSave" class="btn btn-primary save">Save</button>
                    </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    var KTDatatablesServerSide = function () {
        ...
        var initDatatable = function () {
            dt = $("#datatable").DataTable({
                ...
                // scrollX: true,
                order:[[0,'desc']],
                ajax: "{{ route('meta.member-category.datatable') }}",
                columnDefs: [
                {
                    targets: 0,
                    width: '50px',
    ...
    function create(){
        ...
        $('.modal-title').text('Add Data Member Category');
        $('.save').text('Save');
    ...
    function edit(id){
        ...
        $('#form')[0].reset();
        var url = "{{ route('meta.member-category.edit',":id") }}";
        ...
        $.get(url, function (response) {
            ...
            $('#id').val(response.id);
            $('#modal_form').modal('show');
            $('.modal-title').text('Edit Data Member Category');
            $('.save').text('Update');
            $('#btnSave').attr('disabled', false);
    ...
    function submit() {
        ...
        var url = "{{ route('meta.member-category.store') }}";
    
        $('#btnSave').text('Saving...');
        $('#btnSave').attr('disabled', true);

        if(submit_method == 'edit'){
            url = "{{ route('meta.member-category.update',":id") }}";
            url = url.replace(':id', id);
        }
    ...
    function destroy(id) {
        var url = "{{ route('meta.member-category.destroy',":id") }}";
        url = url.replace(':id', id);
