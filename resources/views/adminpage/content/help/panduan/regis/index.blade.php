...
@section('title', 'Panduan | Registrasi - Desa Merdeka')
    ...
    <div class="col-lg-8 col-md-12">
        <div class="card blog-edit">
            ...
            <form id="form" action="{{ route('panduan.store') }}" method="POST">
                <div class="card-body">
                    @csrf
                    <input id="id" name="id" type="hidden" value="{{ isset($registrasi) ? $registrasi->id : '' }}">
                    <input id="type" name="type" type="hidden" value="registrasi">
                    <div class="form-group">
                        <label class="form-label text-dark">Content Registrasi</label>
                        <div id="desc" class="min-h-200px mb-2" >{!! old('desc') ?: (isset($registrasi) ? $registrasi->desc : ' ') !!} </div>                   
                    </div>
                    </div>
                </div>
            ...
            @endrole
            @role('member')
            <div class="card-body {{ optional($registrasi)->desc ? '' : 'text-center'}}">
                {!! optional($registrasi)->desc ?? '<img src="https://plus.unsplash.com/premium_photo-1682309735318-934795084028?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8OXx8ZW1vdGljb258ZW58MHx8MHx8fDA%3D" style = "width:448px" alt=""> <h3 class="mt-3">Its Empty In Here</h3>' !!}
            </div>
            @endrole
        </div>
    </div>
