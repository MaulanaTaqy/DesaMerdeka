...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $("#datatable").DataTable({
            ...
            serverSide: true,
            autoWidth: false,
            ajax: "{{ route('roadmap-banner.datatable') }}",
            columnDefs: [
            {
                targets: 0,
                    ...
                    return `<div class="d-flex align-items-center">
                                <div class="symbol symbol-100px me-5">
                                    <span class="symbol-label bg-light">
                                        <img src="{{ asset('/') }}${data != null? data : 'virtual/assets/img/default.png'}" class="h-75 align-self-end" alt="" />
                                    </span>
                                </div>
                            </div>`
                }
    ...
    function create(){
        ...
        $('.modal-title').text('Add Data Product Bannerrrr');
    }
    ...
    function edit(id){
        ...
        var url = "{{ route('roadmap-banner.edit',":id") }}";
        ...
        $.get(url, function (response) {
            ...
            $('#id').val(response.id);
            $('#modal_form').modal('show');
            $('.modal-title').text('Edit Product Banner ');
    ...
    function submit() {
        ...
        var url = "{{ route('roadmap-banner.store') }}";
        
        $('#btnSave').text('Menyimpan...');
        $('#btnSave').attr('disabled', true);

        if(submit_method == 'edit'){
            url = "{{ route('roadmap-banner.update',':id') }}";
            url = url.replace(':id', id);
    ...
    function destroy(id) {
        var url = "{{ route('roadmap-banner.destroy',":id") }}";
        url = url.replace(':id', id);
