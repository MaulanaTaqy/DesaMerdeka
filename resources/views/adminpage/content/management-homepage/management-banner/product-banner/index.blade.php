...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $("#datatable").DataTable({
            ...
            serverSide: true,
            autoWidth: false,
            ajax: "{{ route('product-banner.datatable') }}",
            columnDefs: [
            {
                targets: 0,
    ...
    function edit(id){
        ...
        var url = "{{ route('product-banner.edit',":id") }}";
    ...
    function submit() {
        ...
        var url = "{{ route('product-banner.store') }}";
        
        $('#btnSave').text('Menyimpan...');
        $('#btnSave').attr('disabled', true);

        if(submit_method == 'edit'){
            url = "{{ route('product-banner.update',':id') }}";
            url = url.replace(':id', id);
    ...
    function destroy(id) {
        var url = "{{ route('product-banner.destroy',":id") }}";
        url = url.replace(':id', id);
                ...
                columnDefs: [
                    ...
                    },             
                    {
                        ...
                        render: function(data, type, full, meta) {
                            return `
                            <div class="btn-list">
                                <a href="javascript:void(0)" onclick="edit('${data}')" class="btn btn-sm btn-primary modal-effect btn-edit" data-bs-effect="effect-super-scaled"><span class="fa-solid fa-pencil"> </span></a>
                                <a href="javascript:void(0)" onclick="destroy('${data}')" class="btn btn-sm btn-danger btn-delete"><span class="fa-solid fa-trash"> </span></a>
                            </div>
                            `;
                        },
                    }
