        ...
        <div class="modal-content">
            ...
            <form class="form" method="POST" id="form">
                ...
                <div class="modal-body py-10 px-lg-17">
                    ...
                    <div class="mt-3 mb-8">
                        <label class="required fs-6 fw-semibold mb-2">Member Type</label>
                        <select class="form-select form-select-solid" id="member_type_id"  data-control="select2" data-placeholder="Select member type" name="member_type_id">
                        </select>
                    </div>
                </div>
                <!--end::Modal body-->
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        ajaxSelect2Initiator("member_type_id", false, "{{ route('member-type-banner.select2') }}");
        ...
        table = $("#datatable").DataTable({
            ...
            serverSide: true,
            autoWidth: false,
            ajax: "{{ route('member-type-banner.datatable') }}",
            columnDefs: [
            {
                targets: 0,
    ...
    function create(){
        ...
        $('#id').val('');
        $('#form')[0].reset();
        $('#member_type_id').empty().trigger('change');
    ...
    function edit(id){
        ...
        var url = "{{ route('member-type-banner.edit',":id") }}";
        ...
        $.get(url, function (response) {
            ...
            var newOption = new Option(response.member_type.name, response.member_type.id, false, false);

            
            $('#id').val(response.id);
            $('#modal_form').modal('show');
            $('.modal-title').text('Edit Blod Banner ');
            $('#member_type_id').append(newOption).trigger('change');
    ...
    function submit() {
        ...
        var url = "{{ route('member-type-banner.store') }}";
        
        $('#btnSave').text('Menyimpan...');
        $('#btnSave').attr('disabled', true);

        if(submit_method == 'edit'){
            url = "{{ route('member-type-banner.update',':id') }}";
            url = url.replace(':id', id);
    ...
    function destroy(id) {
        var url = "{{ route('member-type-banner.destroy',":id") }}";
        url = url.replace(':id', id);
