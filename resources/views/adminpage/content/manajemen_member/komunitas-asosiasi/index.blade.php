...
@section('title', 'Komunitas Asosiasi')
        ...
        <div class="card card-xl-stretch mb-xl-8">
            <div class="card-header border-0 pt-5">
                ...
                <div class="card-toolbar">
                    <a href="{{ url('/komunitas-asosiasi/create') }}" class="btn btn-primary">
                    ...
                    <table id="example" class="table align-middle gs-0 gy-5">
                        <thead>
                            <tr class="fw-bold text-muted bg-light">
                                ...
                                <th class="text-center min-w-125px">Option</th>
                                <th class="text-center min-w-125px">Show</th>
                                <th class="min-w-125px">Name</th>
                                <th class="text-centermin-w-125px">Image</th>
                                <th class="min-w-125px">Email</th>
                                <th class="text-center min-w-150px">Phone</th>
                                <th class="min-w-125px">Registered To</th>
                            </tr>
                        </thead>
                        <tbody>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $("#example").DataTable({
            ...
            serverSide: true,
            scrollX: true,
            ajax: "{{ route('komunitas-asosiasi.datatable') }}",
            columnDefs: [
            {
                targets: 0,
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    if(full.user.permissions.findIndex(item => item.name === 'approved') < 0){
                        state = `<a href="javascript:void(0)" onclick="approve('${full.user.id}')" class="text-center btn btn-icon btn-success btn-approval btn-sm" title="Approve"><span class="fa fa-check"> </span></a>`;
                    }

                    let btn = `
                    <div class="btn-list">
                        ${state}
                        <a href="{{ route('komunitas-asosiasi.edit', ':id') }}" class="text-center btn btn-primary btn-sm" title="Profile Member">Profile</a>
            ...
            }, 
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    if(role == 'admin' || is_can){
                        ...
                        if(!!+full.is_showed)
                            state = `<a href="javascript:void(0)" onclick="uncheck('${data}')" class="text-center btn btn-icon btn-warning btn-sm btn-is-show" title="Hide this item on Homepage"><span class="fa fa-times"> </span></a>`;
                    }
    ...
    function destroy(id) {
        var url = "{{ route('komunitas-asosiasi.destroy',':id') }}";
